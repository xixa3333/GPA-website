...
type Course = { id: number; semester: string; name: string; credits: number; grade: string; gradePoints: number };
...
export default function DashboardClient({ displayName, signOutHref }: { displayName: string; signOutHref: string }) {
  ...
  const summary = useMemo(() => {
    ...
    return { totalCredits, gpa, semesters: new Set(courses.map((course) => course.semester)).size };
  }, [courses]);

  async function addCourse(event: FormEvent) {
    event.preventDefault(); setSaving(true); setError("");
    const response = await fetch("/api/courses", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ semester, name, credits: Number(credits), grade }) });
    const data = await response.json();
    if (response.ok) { setCourses((current) => [data.course, ...current]); setName(""); } else setError(data.error ?? "儲存失敗");
    setSaving(false);
  }
    ...
    <main className="dashboard-page">
      ...
      <div className="dashboard shell">
        ...
        <section className="summary-grid" aria-label="成績摘要">
          ...
          <article className="summary-card"><span>已記錄學期</span><strong>{summary.semesters}</strong><small>semesters</small></article>
        </section>
        <section className="workspace-grid">
          <form className="course-form" onSubmit={addCourse}>
            <div><p className="eyebrow">新增紀錄</p><h2>加入一門課</h2></div>
            ...
            <button className="button primary" disabled={saving}>{saving ? "儲存中…" : "儲存課程"}</button>
          </form>
          <section className="course-list">
            <div className="list-heading"><div><p className="eyebrow">課程清單</p><h2>我的成績</h2></div><span>{courses.length} 門課</span></div>
            {loading ? <p className="empty-state">正在載入你的資料…</p> : courses.length === 0 ? <p className="empty-state">還沒有課程，從左邊新增第一筆紀錄吧。</p> : <div className="table-wrap"><table><thead><tr><th>課程</th><th>學期</th><th>學分</th><th>成績</th><th><span className="sr-only">操作</span></th></tr></thead><tbody>{courses.map((course) => <tr key={course.id}><td>{course.name}</td><td>{course.semester}</td><td>{course.credits}</td><td><span className="grade-badge">{course.grade}</span></td><td><button className="delete-button" onClick={() => void removeCourse(course.id)} aria-label={`刪除 ${course.name}`}>刪除</button></td></tr>)}</tbody></table></div>}
          </section>
        </section>
      </div>
    </main>
