...
export async function GET() {
  ...
  const result = await env.DB.prepare("SELECT id, semester, name, credits, grade, grade_points AS gradePoints, created_at AS createdAt FROM courses WHERE owner_email = ? ORDER BY semester DESC, id DESC").bind(user.email).all();
...
export async function POST(request: Request) {
  const user = await getChatGPTUser();
  if (!user) return Response.json({ error: "請先登入" }, { status: 401 });
  const body = await request.json() as { semester?: string; name?: string; credits?: number; grade?: string };
  ...
  const result = await env.DB.prepare("INSERT INTO courses (owner_email, semester, name, credits, grade, grade_points) VALUES (?, ?, ?, ?, ?, ?) RETURNING id, semester, name, credits, grade, grade_points AS gradePoints, created_at AS createdAt")
    .bind(user.email, semester.slice(0, 30), name.slice(0, 100), credits, grade, gradePoints[grade]).first();
