...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["pdfFile"])) {
    ...
    if (move_uploaded_file($_FILES["pdfFile"]["tmp_name"], $target_file)) {
        $python_executable = getenv('GPA_PYTHON') ?: '';
        if ($python_executable === '' && PHP_OS_FAMILY === 'Windows') {
            $localAppData = getenv('LOCALAPPDATA') ?: '';
            $localPython = $localAppData . '/Programs/Python/Python312/python.exe';
            $python_executable = is_file($localPython) ? $localPython : 'python';
        } elseif ($python_executable === '') {
            $python_executable = 'python3';
        }
        $python_script = __DIR__ . DIRECTORY_SEPARATOR . 'PDF.py';
